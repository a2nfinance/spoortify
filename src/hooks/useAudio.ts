...
const useAudio = (songs) => {
    ...
    const [audio, setAudio] = useState(songs);
    const [trackIndex, setTrackIndex] = useState(0);
    const [newSong, setNewSong] = useState(0);
    const [trackProgress, setTrackProgress] = useState(0);
    const [isPlaying, setIsPlaying] = useState(false);
    const [volume, setVolume] = useState(1);
    const audioRef = useRef(new Audio(resolveLink(audio[trackIndex].songURL)));
    ...
    useEffect(() => {
        ...
        audioRef.current.pause();
        audioRef.current = new Audio(resolveLink(audio[trackIndex].songURL));
